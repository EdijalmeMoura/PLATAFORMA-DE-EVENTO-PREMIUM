...
<title>Esqueci a senha — <?= e($event['name'] ?? 'Evento') ?></title>
...
.okmsg{background:#122a14;border:1px solid #245c2a;color:#b9f0c0;padding:14px;border-radius:8px;font-size:13.5px;line-height:1.6}
...
<div class="card">
  ...
  <p class="sub">Enviaremos um link de redefinição</p>
  <?php if (!empty($error)): ?><div class="err"><?= e($error) ?></div><?php endif; ?>
  <?php if (!empty($sent)): ?>
    <div class="okmsg">Se o e-mail estiver cadastrado e ativo, você receberá o link em instantes (válido por 1 hora). Confira também a caixa de spam.</div>
  <?php else: ?>
  <form method="post">
    <?= App\Core\Csrf::field() ?>
    <label>E-mail de acesso</label>
    <input type="email" name="email" required autofocus placeholder="voce@evento.com">
